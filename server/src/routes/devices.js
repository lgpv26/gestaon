...
module.exports = (server, restify) => {
  ...
  server.use(basePath("/devices", authGuard));

  server.post("/devices", authGuard, (req, res, next) => {
    return server.broker
      .call("data/device.create", {
      ...
      .then(device => {
        server.io.to(`company/${device.companyId}`).emit(
          "tracker:device",
          new EventResponse({
            operation: "add",
            device
