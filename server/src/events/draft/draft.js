...
module.exports = class Draft {
    ...
    _setParentListeners(){
        ...
        vm.socket.instance.on('draft.load', (evData) => {
            ...
            const room = vm.server.io.sockets.adapter.rooms['company/' + vm.socket.activeCompany.id + '/draft/' + evData.draftId]
            if(room.length > 1) return vm.socket.instance.emit('draft.load', new EventResponse(new Error ('Outro atendente já está com este draft aberto!')))
