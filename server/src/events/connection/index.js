...
module.exports = class Connection {
  ...
  _connection() {
    ...
    this.server.rsmq
      ...
      .then(queues => {
        if (!_.includes(queues, "userId-" + this.socket.user.id)) {
          this.server.rsmq
            .createQueue({
              qname: "userId-" + this.socket.user.id,
              maxsize: -1
            })
            .then(() => {
    ...
    this.socket.instance.to("company/" + this.socket.activeCompany.id).emit(
      "presence:add",
      new EventResponse({
    ...
    if (this._rsmqWorkers["userId:" + this.socket.user.id])
      delete this._rsmqWorkers["userId:" + this.socket.user.id];

    this._rsmqWorkers["userId:" + this.socket.user.id] = new RSMQWorker(
      "userId-" + this.socket.user.id,
      {
        maxReceiveCount: 1,
        interval: [0.2, 1, 1.5, 2]
      }
    );

    this._rsmqWorkers["userId:" + this.socket.user.id].on("message", function(
      msg,
      next,
      id
    ) {
      ...
      switch (message.type) {
        case "request":
          return vm.server.broker
            .call(
              "socket.processedQueue",
              _.assign(message, { userId: vm.socket.user.id, messageID: id })
            )
            .then(() => {
              next();
            });
  ...
  _setListeners() {
    ...
    this.socket.instance.on("system:ready", async () => {
      ...
      if (_.includes(usersSystemArray, "userId:" + this.socket.user.id))
        return console.log("Já deu system ready");
      this._usersSystemReady["userId:" + this.socket.user.id] = true;
      console.log(
        "O client do usuario " + this.socket.user.name + " esta pronto!"
      );
      await new Promise((resolve, reject) => {
        this._rsmqWorkers["userId:" + this.socket.user.id].size(
          async (err, size) => {
    ...
    this.socket.instance.on("disconnect", () => {
      if (
        this.socket.user &&
        this._rsmqWorkers["userId:" + this.socket.user.id]
      ) {
      ...
      if (
        this.socket.user &&
        _.includes(usersSystemArray, "userId:" + this.socket.user.id)
      )
        delete this._usersSystemReady["userId:" + this.socket.user.id];
      ...
      if (this.socket.activeCompany)
        this.server.io
          .in("company/" + this.socket.activeCompany.id)
          .emit(
            "presence:remove",
            new EventResponse({
              userId: this.socket.user.id,
