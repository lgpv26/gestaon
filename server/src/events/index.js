    ...
    this.server.io.on("connection", socket => {
      ...
      this.server.mysql.UserAccessToken.findOne({
        ...
        if (userAccessToken && typeof userAccessToken.user !== "undefined") {
          ...
          if (!socket.activeUserCompany)
            socket.activeUserCompany = _.first(socket.user.userCompanies);
          socket.activeCompany = _.find(socket.user.companies, {
            id: socket.activeUserCompany.companyId
          });
          if (!socket.activeCompany)
            socket.activeCompany = _.first(socket.user.companies);
              ...
              return this.server.broker
                ...
                .then(consult => {
                  ...
                  return new Promise((resolve, reject) => {
                    ...
                    socket.instance
                      .to("company/" + socket.activeCompany.id)
                      .emit(
                        "presence:add",
                        new EventResponse({
                          id: socket.user.id,
      ...
      socket.instance.on("disconnect", () => {
        ...
        if (socket.activeCompany)
          this.server.io
            .in("company/" + socket.activeCompany.id)
            .emit("presence:remove", new EventResponse(socket.user.id));
