...
module.exports = class GT06 extends Protocol {
    ...
    timeInDevice(data) {
        return new Promise(async (resolve, reject) => {
            resolve(moment(data.dateInDevice + " " + data.timeInDevice, "DDMMYY HH:mm:ss").toDate())
        })
    }
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if(this.protocol.debug) {
                    console.log("Opa chegou um ping PROTOCOLO GT06 de:", deviceInfos.id, "localização:", data.latitude, data.longitude, "orientação:", data.orientation, "velocidade:", data.speed)
                }
