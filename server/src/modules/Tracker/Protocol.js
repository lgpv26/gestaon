...
module.exports = class PROTOCOL {
    ...
    createServer(){
        return new Promise(async (resolve, reject) => {
            if(!this.protocol ) return reject("Não foi possivel iniciar o servidor do protocolo de rastreamento")
    ...
    netServer(){
        return new Promise(async (resolve, reject) => {
            ...
            log.info(this.protocol.name.toUpperCase() + ' listening on port: ' + this.protocol.port)
            resolve()
        })
    }
    ...
    restifyServer(){
        return new Promise(async (resolve, reject) => {
            ...
            this.protocolServer.use(restify.plugins.acceptParser(this.protocolServer.acceptable))
    ...
    broadcast(position, device){
        return new Promise(async (resolve, reject) => {
            try {
                this.server.io.in('company/' + device.companyId).emit('tracker:position', position)
                return resolve()
            }
            catch(err){
