    ...
    methods: {
      checkTempIds(data) {
        ...
        if (_.has(data, "requestClientAddresses")) {
          removeTemps.push(
            this.removeArrayTempIds(data, "requestClientAddresses").then(
        ...
        _.map(_.get(data, path), obj => {
          promises.push(
            new Promise((resolve, reject) => {
              ...
              if (
                _.get(obj, "requestId", false) &&
                !_.isNumber(obj.requestId) &&
                obj.requestId.substring(0, 4) === "tmp/"
              ) {
                obj.requestId = null;
              }
              newValue.push(obj);
          ...
          detailsData.requestClientAddresses.forEach(
            (requestClientAddress, index) => {
              ...
              if (
                ...
                !_.isNumber(requestClientAddress.clientAddressId)
              ) {
                clientAddress = _.find(
              ...
              if (!_.isNumber(requestClientAddress.requestId))
                _.set(requestClientAddress, "requestId", request.id);
