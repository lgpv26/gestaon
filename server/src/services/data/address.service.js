        ...
        return new Promise((resolve, reject) => {
          ...
          return new Promise((resolve, reject) => {
            ...
            data.forEach((forEachAddress, index) => {
              if (
                _.has(forEachAddress.address, "id") &&
                forEachAddress.address.id
              ) {
                ...
                data[index].address.companyId = parseInt(
                  ctx.params.data.companyId
                );
              }
            });

            ctx
              .call("data/address.list", {
                where: {
                  id: {
                    [Op.in]: addressesIds
                  }
              ...
              .then(addressesConsult => {
                addressesConsult.forEach(result => {
                  ...
                  if (result.companyId === 0) {
                    addressCantChange.push(
                      _.assign(data[index], {
                        address: JSON.parse(JSON.stringify(result))
                      })
                    );
                    data.splice(index, 1);
                  }
            ...
            data.forEach(clientAddress => {
              if (clientAddress.address.id) {
                addressChangePromises.push(
                  ctx
                    .call("data/address.update", {
            ...
            return Promise.all(addressChangePromises)
              .then(addresses => {
                resolve(addresses);
              })
              .catch(err => {
          ...
          .then(response => {
            return _.concat(
              response ? response : [],
              addressCantChange ? addressCantChange : []
            );
          })
          .catch(err => {
