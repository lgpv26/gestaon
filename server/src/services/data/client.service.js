        ...
        return new Promise((resolve, reject) => {
          async function start() {
            try {
              ...
              const transaction = await vm.checkTransaction(
                ctx.params.transaction || null
              );
              ...
              if (_.has(ctx.params.data, "clientAddresses")) {
                _.set(
                  client,
                  "clientAddresses",
                  await vm.setClientAddresses(
                    ctx.params.data.clientAddresses,
                    client.id,
                    companyId,
                    transaction
                  )
              ...
              if (_.has(ctx.params.data, "clientPhones")) {
                const clientPhones = _.map(
                  ctx.params.data.clientPhones,
                  clientPhone => {
              ...
              if (_.has(ctx.params.data, "clientCustomFields")) {
                const clientCustomFields = _.map(
                  ctx.params.data.clientCustomFields,
                  clientCustomField => {
                ...
                _.set(
                  client,
                  "clientCustomFields",
                  await vm.saveClientCustomFields(
                    clientCustomFields,
                    client.id,
                    transaction
                  )
    ...
    methods: {
      ...
      checkTempIds(data, request) {
        ...
        if (_.has(data, "clientCustomFields")) {
          removeTemps.push(
            this.removeArrayTempIds(data, "clientCustomFields").then(
              clientCustomFields => {
          ...
          promises.push(
            new Promise((resolve, reject) => {
              ...
              if (
                ...
                obj.address &&
                obj.address.id &&
                !_.isNumber(obj.address.id) &&
                obj.address.id.substring(0, 4) === "tmp/"
              ) {
      ...
      setClientAddresses(data, clientId, companyId, transaction) {
        return server.broker
          .call("data/address.saveAddresses", {
            data,
            companyId,
            transaction
          })
          .then(clientAddressWithAddress => {
            ...
            return this.saveClientAddresses(
          ...
          data.forEach(clientAddress => {
            if (clientAddress.id) {
              clientAddressesPromises.push(
                server.mysql.ClientAddress.update(
                  ...
                  if (parseInt(_.toString(clientAddressUpdate)) < 1) {
                    console.log(
                      "Nenhum registro encontrado. Update. clientAddressUpdate"
                    );
                    return Promise.reject(
                      "Erro ao atualizar endereço do cliente."
                    );
                  }
                  return server.mysql.ClientAddress.findById(clientAddress.id, {
                    transaction
                  }).then(ClientAddress => {
                    return _.assign(JSON.parse(JSON.stringify(ClientAddress)), {
                      type: clientAddress.type,
                      select: clientAddress.select,
              ...
              clientAddressesPromises.push(
                server.mysql.ClientAddress.create(clientAddress, {
                  ...
                  return _.assign(
                    JSON.parse(JSON.stringify(clientAddressCreate)),
                    {
                      type: clientAddress.type,
                      select: clientAddress.select,
                      tmpId: clientAddress.tmpId,
                      address: clientAddress.address
                    }
                  );
                })
        ...
        return server.mysql.ClientPhone.destroy({
          ...
          data.forEach(clientPhone => {
            if (clientPhone.id) {
              clientPhonesPromises.push(
                server.mysql.ClientPhone.update(
                  ...
                  if (parseInt(_.toString(clientPhoneUpdate)) < 1) {
                    console.log(
                      "Nenhum registro encontrado. Update. clientPhone"
                    );
                    return Promise.reject(
                  ...
                  return server.mysql.ClientPhone.findById(clientPhone.id, {
                    ...
                    return _.assign(
                      JSON.parse(JSON.stringify(clientPhoneUpdated)),
                      { select: clientPhone.select }
                    );
                  });
                })
              );
              ...
              clientPhonesPromises.push(
                server.mysql.ClientPhone.create(clientPhone, {
                  ...
                  if (!clientPhoneCreate) {
                    console.log(
                      "Nenhum registro encontrado. Create clientPhone."
                    );
                    return Promise.reject(
                      "Erro ao cadastrar telefone do cliente."
                    );
                  }
                  return _.assign(
                    JSON.parse(JSON.stringify(clientPhoneCreate)),
                    { select: clientPhone.select, tmpId: clientPhone.tmpId }
                  );
                })
              );
            }
          ...
          return Promise.all(clientPhonesPromises)
            ...
            .catch(err => {
              console.log(err);
              console.log(
                "Erro em: data/client.saveClientPhones - Promise ALL"
              );
      ...
      saveClientCustomFields(data, clientId, transaction) {
        ...
        return server.mysql.ClientCustomField.destroy({
              ...
              clientCustomFieldsPromises.push(
                server.mysql.ClientCustomField.update(
                  ...
                  if (parseInt(_.toString(clientCustomFieldUpdate)) < 1) {
                    console.log(
                      "Nenhum registro encontrado. Update. clientCustomField"
                    );
                    return Promise.reject(
                      "Erro ao atualizar dados do cliente."
                    );
                  }
                  return server.mysql.ClientCustomField.findById(
                    clientCustomField.id,
                    { transaction }
              ...
              clientCustomFieldsPromises.push(
                server.mysql.ClientCustomField.create(clientCustomField, {
                  ...
                  if (!clientCustomFieldCreate) {
                    console.log(
                      "Nenhum registro encontrado. Create clientCustomField."
                    );
                    return Promise.reject(
                      "Erro ao cadastrar dados do cliente."
                    );
                  }
                  return _.assign(
                    JSON.parse(JSON.stringify(clientCustomFieldCreate)),
                    { tmpId: clientCustomField.tmpId }
                  );
                })
              );
            }
          ...
          return Promise.all(clientCustomFieldsPromises)
            ...
            .catch(err => {
              console.log(
                "Erro em: data/client.saveClientCustomFields - Promise ALL"
              );
              return Promise.reject(
                "Erro ao interagi com os dados do cliente."
              );
            });
        });
      }
