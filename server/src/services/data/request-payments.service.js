...
module.exports = server => {
  return {
        ...
        return new Promise((resolve, reject) => {
          async function start() {
            try {
              ...
              const removedRequestPayments = await vm.checkRemovedRequestPayments(
                ctx.params.data,
                oldRequest,
                client ? client : null
              );
              const dataRequestPayments = await vm.checkChanges(
                ctx.params.data,
                request,
                oldRequest
              );

              await vm.revertPayments(
                removedRequestPayments,
                dataRequestPayments,
                userId,
                transaction
              );

              await vm.clientLimit(
                client,
                removedRequestPayments,
                dataRequestPayments,
                transaction
              ...
              const requestPayments = await vm.requestPayments(
                dataRequestPayments,
                removedRequestPayments,
                request,
                userId,
    ...
    methods: {
      changeTempIdDEV(data) {
        // DEVELOP
        const promises = [];
        data.forEach((payment, index) => {
          promises.push(
            new Promise((resolve, reject) => {
              switch (payment.tmpId) {
                case "tmp/L1bwqQUgzq":
                  _.set(payment, "id", 319);
                  _.set(
                    payment,
                    "paid",
                    _.toString(payment.amount).substring(4) == "1"
                      ? true
                      : false
                  );
                  _.set(
                    payment,
                    "deadlineDatetime",
                    _.toString(payment.amount).substring(3) == "1"
                      ? "2019-01-15T22:58:07.155Z"
                      : null
                  );
                  _.set(
                    payment,
                    "amount",
                    parseFloat(_.toString(payment.amount).substring(0, 2))
                  );
                  console.log("SWITCH APENAS PARA DEV - REMOVER!");
                  break;
                case "tmp/OB7HccASVs":
                  _.set(payment, "id", 320);
                  _.set(
                    payment,
                    "paid",
                    _.toString(payment.amount).substring(4) == "1"
                ...
                //return Promise.reject('FODEU')
              }
              resolve(payment);
            })
          );
        });
        return Promise.all(promises).then(temps => {
          return Promise.resolve(temps);
        });
      },

      fakeConsult(request, transaction) {
        return server.mysql.Account.findById(5, {
          transaction
        }).then(request => {
          //console.log(JSON.parse(JSON.stringify(request)))
          return Promise.reject("ESSA TA QUASE EM!");
        });
      },

      checkRemovedRequestPayments(data, oldRequest, client) {
        const oldRequestPayments =
          oldRequest && _.has(oldRequest, "requestPayments")
            ? oldRequest.requestPayments
            : null;
        if (!oldRequestPayments) return Promise.resolve(null);
        const removedRequestPayments = _.differenceBy(
          oldRequestPayments,
          data,
          "id"
        );
            ...
            promises.push(
              new Promise((resolve, reject) => {
                if (requestPayment.settled)
                  return reject(
                    "Não é possível remover o pagamento " +
                      requestPayment.paymentMethod.name +
                      " no valor de R$ " +
                      parseFloat(requestPayment.amount) +
                      ' por já estar marcado como "Acertado"!'
                  );
      ...
      checkChanges(data, request, oldRequest) {
        ...
        data.forEach((payment, index) => {
          promises.push(
            new Promise((resolve, reject) => {
              const indexOldPayment = _.findIndex(
                oldRequestPayments,
                oldPayment => {
              ...
              if (
                ...
                parseFloat(payment.amount) !==
                  parseFloat(oldRequestPayments[indexOldPayment].amount)
              ) {
                ...
                _.set(
                  payment,
                  "oldPaid",
                  oldRequestPayments[indexOldPayment].paid
                );
                _.set(
                  payment,
                  "oldDeadlineDatetime",
                  oldRequestPayments[indexOldPayment].deadlineDatetime
                );
                _.set(
                  payment,
                  "oldPaymentMethod",
                  oldRequestPayments[indexOldPayment].paymentMethod
                );
                _.set(
                  payment,
                  "oldResponsibleUser",
                  oldRequest.responsibleUser
                );
                _.set(
                  payment,
                  "requestPaymentTransactions",
                  oldRequestPayments[indexOldPayment].requestPaymentTransactions
                );
              ...
              if (
                indexOldPayment !== -1 &&
                payment.paid !== oldRequestPayments[indexOldPayment].paid
              ) {
              ...
              if (
                ...
                payment.paymentMethodId !==
                  oldRequestPayments[indexOldPayment].paymentMethodId
              ) {
              ...
              if (
                indexOldPayment !== -1 &&
                request.responsibleUser.id !== oldRequest.responsibleUser.id
              ) {
                //console.log('METHOD', index, payment.paymentMethodId, oldRequestPayments[indexOldPayment].paymentMethodId, (payment.paymentMethodId !== oldRequestPayments[indexOldPayment].paymentMethodId))
              ...
              if (
                indexOldPayment !== -1 &&
                oldRequestPayments[indexOldPayment].settled &&
                (payment.changedPaid ||
                  payment.changedAmount ||
                  payment.changedMethod)
              ) {
                const paid = payment.changedPaid ? "o valor" : "";
                const amount = payment.changedAmount ? 'o status "PAGO"' : "";
                const method = payment.changedMethod
                  ? "o tipo de pagamento de " +
                    payment.paymentMethod.name +
                    " para " +
                    oldRequestPayments[indexOldPayment].paymentMethod.name
                  : "";
                const virgula1 =
                  payment.changedAmount || payment.changedMethod ? " e " : "";
        ...
        return Promise.all(promises).then(changeds => {
      ...
      revertPayments(
        removedRequestPayments,
        requestPaymentsChange,
        userId,
        transaction
      ) {
        const revertRemovedRequestPayments = _.filter(
          removedRequestPayments,
          removedRequestPayment => {
            return removedRequestPayment.paid;
          }
        );

        const requestPaymentsChanges = _.filter(
          requestPaymentsChange,
          requestPayment => {
            ...
            if (
              requestPayment.changedResponsableUser &&
              requestPayment.oldPaid
            ) {
              return requestPayment;
            }
          }
        );
        const revertsRequestPayments = _.concat(
          revertRemovedRequestPayments,
          requestPaymentsChanges
        );
        ...
        revertsRequestPayments.forEach(requestPayment => {
          if (requestPayment.oldPaid) {
            ...
            const requestTransaction = _.first(
              requestPayment.requestPaymentTransactions
            );
            promises.push(
              new Promise((resolve, reject) => {
                if (!requestTransaction.revert) {
                  return server.broker
                    .call("data/transaction.create", {
                      data: {
                        amount: -Math.abs(
                    ...
                    .then(transactionCreate => {
                      return server.mysql.Account.findById(
                        ...
                        accountBalances[account.id] =
                          parseFloat(accountBalances[account.id]) +
                          parseFloat(transactionCreate.amount);
                ...
                return reject(
                  "Erro ao reverter o pagamento #" +
                    requestPayment.id +
                    " não possível confirmar a transação anterior! Favor consultar suporte e informar pedido: #" +
                    requestPayment.request.id
                );
              })
            );
        ...
        return Promise.all(promises).then(() => {
          ...
          return Promise.all(updateAccountBalancesPromise).catch(err => {
            console.log(
              'ERRO: NO BALANCES E REVERT DO PAGAMENTO! "request=payments.service.js"',
              err
            );
            return Promise.reject(
              "ERRO: No registro do pagamento! Favor contatar o suporte!"
            );
          });
        });
      },
      ...
      clientLimit(
        ...
        const creditLimitRequired = _.filter(
          requestPayments,
          requestPayment => {
            if (
              !requestPayment.paid &&
              !requestPayment.paymentMethod.autoPay &&
              requestPayment.deadlineDatetime
            )
              return requestPayment;
          }
        );

        if (!client && creditLimitRequired.length)
          return Promise.reject(
            'Não é possivel realizar uma venda com a forma de pagamento "NOTINHA" sem cliente selecionado!'
          );
        if (!client) return Promise.resolve();

        const reverseRemovedRequestPayments = _.filter(
          removedRequestPayments,
          removedRequestPayment => {
            if (
              !removedRequestPayment.paid &&
              !removedRequestPayment.paymentMethod.autoPay &&
              removedRequestPayment.deadlineDatetime
            )
              return removedRequestPayment;
          }
        );

        const reversePaymentsChanges = _.filter(
          requestPayments,
          requestPayment => {
            if (
              (requestPayment.changedPaid ||
                requestPayment.changedAmount ||
                requestPayment.changedMethod) &&
              ((requestPayment.changedMethod &&
                !requestPayment.oldPaymentMethod.autoPay) ||
                !requestPayment.oldPaymentMethod.autoPay) &&
              ((requestPayment.changedPaid && requestPayment.oldPaid) ||
                !requestPayment.oldPaid)
            ) {
              return requestPayment;
            }
        ...
        const reverseLimitRequestPayments = _.concat(
          reverseRemovedRequestPayments,
          reversePaymentsChanges
        );

        return new Promise((resolve, reject) => {
          if (reverseLimitRequestPayments.length) {
            const clientLimitInUse = _.sumBy(
              reverseLimitRequestPayments,
              "oldAmount"
            );
            return server.mysql.Client.update(
              {
                limitInUse:
                  parseFloat(client.limitInUse) - parseFloat(clientLimitInUse)
              },
              {
                where: {
                  id: client.id
              ...
              if (!parseInt(_.toString(limiteUpdated)))
                return reject(
                  "Não foi possível alterar o limite de credito em uso do cliente!"
                );
              return resolve();
            });
          }
          ...
          if (creditLimitRequired.length) {
            return server.mysql.Client.findById(client.id, {
              ...
              const newLimitInUse =
                parseFloat(clientConsult.limitInUse) +
                parseFloat(_.sumBy(creditLimitRequired, "amount"));
              const checkLimit = _.gte(
                parseFloat(clientConsult.creditLimit),
                newLimitInUse
              );

              if (!checkLimit)
                return Promise.reject(
                  "Cliente não tem o limite de credito necessario disponivel para realizar este pedido!"
                );
              ...
              return server.mysql.Client.update(
                ...
                if (!parseInt(_.toString(newLimiteUpdated)))
                  return Promise.reject(
                    "Não foi possível alterar o limite de credito em uso do cliente!"
                  );
        ...
        return server.mysql.RequestPayment.destroy({
          ...
          data.forEach(requestPayment => {
            if (requestPayment.id) {
                ...
                server.mysql.RequestPayment.update(
                  _.assign(requestPayment, {
                    deadlineDatetime: requestPayment.deadlineDatetime
                      ? requestPayment.deadlineDatetime
                      : null,
                    paidDatetime: requestPayment.paidDatetime
                      ? requestPayment.paidDatetime
                      : moment(),
                    dateRemoved: null
                  }),
                  {
                  ...
                  if (parseInt(_.toString(paymentMethodUpdate)) < 1)
                    return Promise.reject(
                      "Erro ao atualizar a forma de pagamento"
                    );
                  return server.mysql.RequestPayment.findById(
                    requestPayment.id,
                    { transaction }
                  ).then(paymentMethod => {
                    ...
                    return Promise.resolve(
                      _.assign(paymentMethod, {
                        tmpId: requestPayment.tmpId
                          ? requestPayment.tmpId
                          : null
                      })
                    );
                  });
              ...
              paymentMethodsPromises.push(
                server.mysql.RequestPayment.create(
                  _.assign(requestPayment, {
                    deadlineDatetime: requestPayment.deadlineDatetime
                      ? requestPayment.deadlineDatetime
                      : null,
                  ...
                  if (!paymentMethod)
                    return Promise.reject(
                      "Erro ao criar a forma de pagamento."
                    );
                  paymentMethod = JSON.parse(JSON.stringify(paymentMethod));
                  if (paymentMethod.paid) paymentsPaids.push(paymentMethod);
                  return Promise.resolve(
                    _.assign(paymentMethod, {
                      tmpId: requestPayment.tmpId ? requestPayment.tmpId : null
                    })
                  );
                })
              );
            ...
            if (paymentsPaids.length) {
                    ...
                    return server.broker
                      .call("data/transaction.create", {
                        data: {
                          ...
                          description:
                            "Adição do valor do pagamento do pedido #" +
                            request.id +
                            " na conta de destino"
                        },
                        transaction
              ...
              return Promise.all(promises).then(() => {
                ...
                return Promise.all(updateAccountBalancesPromise)
                  ...
                  .catch(err => {
                    console.log(
                      'ERRO: NO BALANCES DO PAGAMENTO! "request=payments.service.js"',
                      err
                    );
                    return Promise.reject(
                      "ERRO: No registro da transação do pagamento! Favor contatar o suporte!"
                    );
                  });
              });
            }
