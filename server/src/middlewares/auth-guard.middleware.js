...
module.exports = (server, restify) => {
    ...
    server.mysql.UserAccessToken.findOne({
      ...
      if (moment(userAccessToken.expiresAt).isBefore(moment())) {
        const error = new HTTPError(
          "Token is expired. Try using the refresh_token grant",
          401
        );
      ...
      if (_.has(req.query, "companyId")) {
        const verifyResult = userBelongsToRequestedCompany(
          userAccessToken.user,
          req.query.companyId
        );
        if (!verifyResult) {
          const error = new HTTPError(
            "The authenticated user doesn't belong to the requested company",
            401
          );
  ...
  function userBelongsToRequestedCompany(user, requestedCompanyId) {
    ...
    const belongsTo = _.find(user.userCompanies, userCompany => {
      if (parseInt(userCompany.company.id) === parseInt(requestedCompanyId))
        return true;
    });
    return !!belongsTo;
  }
