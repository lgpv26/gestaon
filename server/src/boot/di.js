...
module.exports = class DependencyInjection {
  ...
  setFCM() {
    ...
    this.server.fcm = new FCM(
      "AAAAZ0yr-pI:APA91bGSFvvNFBC3wL0fkyjtrZYu592dvCU10-6tqMTm2-5PZLUQcksnhZ8LYU0LaVxEgREU3FVxPh2_BOzt-mFkZ1nXgU5UcG8p838HiUSSYPDMkTVypS3EC37JIhcLI6XOL5GllczZ"
    );
  ...
  gpsProtocols() {
    return new Promise(async (resolve, reject) => {
      ...
      config.protocols.forEach(protocol => {
        protocol["instance"] = new (require("../modules/Tracker/protocols/" +
          protocol.name))(this.server, protocol);
        promises.push(protocol.instance.init());
      });
  ...
  initializeSentry() {
    ...
    Sentry.init({
      dsn: "https://f51d206d28ea4d01b89bb2b22131f6ba@sentry.io/1419760"
    });
  }
  ...
  setSequelize() {
    ...
    this.server.sequelize = new Sequelize(
  ...
  async cronJob() {
      ...
      if (resultKeys.length) {
        resultKeys.forEach(key => {
          this.server.redisClient.get(key, async (err, cronJob) => {
            ...
            this.server.broker.call("cronJob.createChatJob", {
              cronJobName: key,
    ...
    await this.server.broker.call("cronJob.backup", {
      rotines: ["*/15 8-20 * * *", "*/30 21-23 * * *", "*/60 0-7 * * *"]
    });
  ...
  brokerCreateService() {
    ...
    return new Promise((resolve, reject) => {
      async function start() {
        const indexPath = await fs
          .readdirSync(path.join(__dirname, "../services"))
          .filter(fileName => {
            return (
              fileName.substring(fileName.length, fileName.length - 6) !==
                "BKP.js" &&
              fileName.substring(fileName.length, fileName.length - 3) == ".js"
            );
          })
          .map(file => {
        ...
        const dataPath = await fs
          .readdirSync(path.join(__dirname, "../services/data"))
          .filter(fileName => {
            return (
              fileName.substring(fileName.length, fileName.length - 6) !==
              "BKP.js"
        ...
        const apiExternal = await fs
          .readdirSync(path.join(__dirname, "../services/apiExternal"))
          .filter(fileName => {
            return (
              fileName.substring(fileName.length, fileName.length - 6) !==
              "BKP.js"
          ...
          .map(file => {
            return (
              "../services/apiExternal/" + file.substring(0, file.length - 3)
            );
          });
        ...
        _.concat(indexPath, dataPath, apiExternal).forEach(service => {
          promises.push(
            vm.server.broker.createService(require(service)(vm.server))
          );
        });
