...
export default {
    ...
    formatPhone(phoneNumber) {
        ...
        if (number.length === 8) {
            return (
                "(" + ddd + ") " + number.substring(0, 4) + "-" + number.substring(4, 8)
            );
    ...
    getShortTime(seconds, pluralize = false) {
        ...
        if (minutes < 60) {
            ...
            return {
                value: months,
                abbUnit:
                (months <= 1 ? "mês" : "mes") + (months > 1 && pluralize ? "." : ""),
                unit:
                (months <= 1 ? "mês" : "mes") + (months > 1 && pluralize ? "es" : "")
            };
    ...
    getShortString(str, max, add, ifFalseStr = "S/N") {
        if (!str) {
            return ifFalseStr;
        }
        add = add || "...";
        return typeof str === "string" && str.length > max
            ? str.substring(0, max).trim() + " " + add
            : str.trim();
    },
    ...
    isDistanceViableThroughSpeed(speed, timeDifference, distance) {
        if (distance < (speed / 3.6) * timeDifference) return true;
        else return false;
    },
