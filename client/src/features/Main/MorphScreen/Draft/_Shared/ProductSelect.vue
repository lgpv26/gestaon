        ...
        <template slot="content">
            <div style="width: 240px;">
                ...
                <div v-for="(product, index) in products" :key="index" class="item">
                    <div style="margin-top: 10px; position: relative;" v-if="editing === product.id">
                    ...
                    <div v-else :class="{ active: value.id === product.id }" style="display: flex; flex-direction: row;">
                        <span style="cursor: pointer;" @click="select(product)">{{ product.name }}</span>
                        <span class="push-both-sides"></span>
                        <icon-check style="height: 11px;"></icon-check>
                        <a href="javascript:void(0)" v-if="false" style="margin-right: 3px;" @click="edit(product)">
                            <icon-edit></icon-edit>
                        </a>
                    </div>
                </div>
