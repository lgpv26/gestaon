<template>
  <div class="container__request-board__filter">
    <div
      class="request-board__filter  filter--date"
      :offset="{ bottom: 15 }"
      data-toggle
    >
      <div class="filter-item__target">
        ...
        <div class="target__amount">
          ...
          <app-datetime-selector
            ...
            @on-change="onFilterChange($event)"
            placeholder="..."
          ></app-datetime-selector>
        </div>
      </div>
    ...
    <app-select
      ...
      :value="filters.clientGroups"
      :popoverProps="{ placement: 'bottom-start' }"
      :title="'Grupo do cliente'"
      @unselect="
        SET_FILTER({ type: 'clientGroups', select: false, id: $event })
      "
      @select="SET_FILTER({ type: 'clientGroups', select: true, id: $event })"
      :items="selectClientGroups"
      :multiple="true"
      :verticalOffset="15"
    >
      <div class="filter-item__target">
        ...
        <div class="target__amount" v-if="filters.clientGroups.length">
          ...
          <span v-if="filters.clientGroups.length === 1">
            {{
              _.find(selectClientGroups, {
                value: _.first(filters.clientGroups)
              }).text
            }}
          </span>
    ...
    <app-select
      ...
      :title="'Responsável'"
      @unselect="
        SET_FILTER({ type: 'responsibleUsers', select: false, id: $event })
      "
      @select="
        SET_FILTER({ type: 'responsibleUsers', select: true, id: $event })
      "
      :multiple="true"
      :items="selectUsers"
      :verticalOffset="15"
      ...
      <div class="filter-item__target">
        ...
        <div class="target__amount" v-if="filters.responsibleUsers.length">
          ...
          <span v-if="filters.responsibleUsers.length === 1">
            {{
              _.find(selectUsers, { value: _.first(filters.responsibleUsers) })
                .text
            }}
          </span>
...
export default {
  ...
  computed: {
    ...
    ...mapFields(["filters.deliveryDate"]),
    selectClientGroups() {
